...
class BoteFelicitupBloc extends Bloc<BoteFelicitupEvent, BoteFelicitupState> {
  BoteFelicitupBloc({required FelicitupRepository felicitupRepository})
    ...
    on<BoteFelicitupEvent>(
      (events, emit) => events.map(
        ...
        updateFelicitupBote: (event) =>
            _updateFelicitupBote(emit, event.felicitupId),
  ...
  void _changeLoading(Emitter<BoteFelicitupState> emit) {
    emit(state.copyWith(isLoading: !state.isLoading));
  }

  void _setBoteQuantity(Emitter<BoteFelicitupState> emit, int quantity) {
    emit(state.copyWith(boteQuantity: quantity));
  }

  Future<void> _updateFelicitupBote(
    Emitter<BoteFelicitupState> emit,
    String felicitupId,
  ) async {
    emit(state.copyWith(isLoading: true));
    try {
      final response = await _felicitupRepository.updateBoteFelicitup(
        felicitupId,
        state.boteQuantity!,
      );
      response.fold(
  ...
  void _startListening(Emitter<BoteFelicitupState> emit, String felicitupId) {
