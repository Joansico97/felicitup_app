...
class InfoFelicitupMobilePage extends StatefulWidget {
  const InfoFelicitupMobilePage({super.key});

  @override
  State<InfoFelicitupMobilePage> createState() =>
      _InfoFelicitupMobilePageState();
}

class _InfoFelicitupMobilePageState extends State<InfoFelicitupMobilePage> {
  @override
  void initState() {
    super.initState();
    WidgetsBinding.instance.addPostFrameCallback((_) {
      if (detailsFelicitupNavigatorKey.currentContext != null && mounted) {
        detailsFelicitupNavigatorKey.currentContext!
            ...
            .add(const DetailsFelicitupDashboardEvent.changeCurrentIndex(0));
      }
    });
  }
          ...
          >(
            builder: (context, dashboardState) {
              ...
              if (felicitup == null) {
                return Center(
                  child: Text(
                    "Cargando detalles del Felicitup...",
                    style: context.styles.paragraph,
                  ),
                );
              }
                  ...
                  children: [
                    ...
                    BlocBuilder<InfoFelicitupBloc, InfoFelicitupState>(
                      builder: (_, state) {
                        return DetailsRow(
                          onTap: () {
                            final bloc = context.read<InfoFelicitupBloc>();
                            List<UserModel> friendListForModal = [
                              ...state.friendList,
                            ];
