...
class ProfileBloc extends Bloc<ProfileEvent, ProfileState> {
  ProfileBloc({required UserRepository userRepository})
    ...
    on<ProfileEvent>(
      (events, emit) => events.map(
        ...
        updateUserImageFromUrl: (event) =>
            _updateUserImageFromUrl(emit, event.url),
        updateUserInfo: (event) => _updateUserInfo(emit, event.user),
        updateUserImageFromFile: (event) =>
            _updateUserImageFromFile(emit, event.file),
      ),
    );
  }
  ...
  Future<void> _updateUserImageFromUrl(
    ...
    try {
      ...
      response.fold(
        ...
        (r) => emit(
          state.copyWith(isLoading: false, status: ProfileStatus.success),
        ),
      );
    } catch (e) {
  ...
  Future<void> _updateUserImageFromFile(
    Emitter<ProfileState> emit,
    File file,
  ) async {
    emit(state.copyWith(isLoading: true));
